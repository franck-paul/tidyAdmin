<?php
declare(strict_types=1);

/**
 * @package Dotclear
 *
 * @copyright Olivier Meunier & Association Dotclear
 */
#
# DOT NOT MODIFY THIS FILE !
#

use Dotclear\Helper\L10n;

L10n::$locales['Tidy Administration'] = '';
L10n::$locales['Customize your dotclear administration'] = '';
L10n::$locales['Go to dashboard'] = '';
L10n::$locales['Options updated'] = '';
L10n::$locales['Unable to write file %s. Please check the dotclear var folder permissions.'] = '';
L10n::$locales['JS supplemental script updated'] = '';
L10n::$locales['CSS supplemental rules updated'] = '';
L10n::$locales['PO supplemental l10n updated'] = '';
L10n::$locales['HTML head supplemental directives updated'] = '';
L10n::$locales['System'] = '';
L10n::$locales['Tidy administration settings'] = '';
L10n::$locales['Options'] = '';
L10n::$locales['These options will be set only for the current user'] = '';
L10n::$locales['Header'] = '';
L10n::$locales['Use mini Dotclear icon (top left) in header'] = '';
L10n::$locales['Move the search form (main menu) in header'] = '';
L10n::$locales['Double click on header to switch theme (not permanent)'] = '';
L10n::$locales['To permanently change theme go the your preferences'] = '';
L10n::$locales['Add a button in header to stop or run Javascript fetch requests (not permanent)'] = '';
L10n::$locales['Use an user-defined background color for header'] = '';
L10n::$locales['Backgound color for header:'] = '';
L10n::$locales['Backgound color for header (dark mode):'] = '';
L10n::$locales['Menu and dock'] = '';
L10n::$locales['Enabled mouse hover activation on collapser'] = '';
L10n::$locales['Display dock with favorites'] = '';
L10n::$locales['The dock will be hidden on small screens'] = '';
L10n::$locales['Report the currently active favorite in the dock'] = '';
L10n::$locales['Automatically hide/show the dock'] = '';
L10n::$locales['The dock will be hidden/shown only if hovering over it is possible'] = '';
L10n::$locales['Media manager'] = '';
L10n::$locales['Clone the media manager search input in always visible area'] = '';
L10n::$locales['Add an exchange button between alternative text and media description'] = '';
L10n::$locales['Theme editor'] = '';
L10n::$locales['Minify theme resources modified by the theme editor'] = '';
L10n::$locales['Set theme editor in development mode'] = '';
L10n::$locales['In development mode, custom files will be ignored and theme files will be editable'] = '';
L10n::$locales['Tools'] = '';
L10n::$locales['Move plugin settings link to top of page'] = '';
L10n::$locales['Always display editor toolbar during edition'] = '';
L10n::$locales['Display direct access menus on blog parameters page'] = '';
L10n::$locales['Display direct access menus on preferences page'] = '';
L10n::$locales['Keep the list of defined widgets visible while scrolling (widget management page)'] = '';
L10n::$locales['Save'] = '';
L10n::$locales['Supplemental CSS'] = '';
L10n::$locales['Note: this supplemental CSS rules will surcharge the default CSS rules.'] = '';
L10n::$locales['Sample CSS:'] = '';
L10n::$locales['Supplemental JS'] = '';
L10n::$locales['Note: this supplemental JS script will surcharge the default JS scripts.'] = '';
L10n::$locales['Sample JS:'] = '';
L10n::$locales['Supplemental PO'] = '';
L10n::$locales['Note: this supplemental PO l10n will surcharge the default l10n.'] = '';
L10n::$locales['Sample PO:'] = '';
L10n::$locales['Supplemental HTML head directives'] = '';
L10n::$locales['Note: this supplemental HTML head directives will added to the default HTML head.'] = '';
L10n::$locales['Sample HTML head:'] = '';
